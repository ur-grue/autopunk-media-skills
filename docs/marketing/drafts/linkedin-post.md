# LinkedIn Post (EN)

I've open-sourced 354 Claude skills built for media professionals.

TV producers, journalists, podcasters, YouTubers, radio producers, newsletter writers, PR teams, screenwriters — if you write for a living, there's probably a skill in here that saves you an hour.

Each skill turns Claude into a specialist for one specific task: pitch treatments, interview questions, show notes, press releases, SEO descriptions, image prompts, data story angles, crisis statements, and more.

Quality-tested: 226 skills evaluated across 2,390 checks. 98.5% pass rate.

3 examples of what the output actually looks like:

A documentary logline:
"In a basement lab outside Washington, three archivists race to recover government records that were officially destroyed — before the statute of limitations runs out on the people who ordered the erasure."

A YouTube hook:
"94% of adult language learners plateau within six months. Not because they quit. Because they're doing the one thing every expert recommends — and it is actively working against how the adult brain acquires language."

A press release opening:
"Seven years in production. Forty-two filming locations across three continents. One question that ornithologists have debated for a century: where do they actually go?"

No coding required. Clone the repo or copy-paste any skill into Claude.

Free. MIT licensed. Built from 15+ years of production experience.

→ https://github.com/ur-grue/autopunk-media-skills

#MediaProduction #AI #ClaudeAI #Journalism #Documentary #Podcasting #YouTube #ContentCreation #OpenSource
